<div class="final_content_wrap">
	...
	<?php if ( false && _DEMO_MODE ) { ?>
		<div style="float:right; padding:5px; font-style: italic;">
			<strong>UCM Demonstration Website</strong> <br/> This is a demo website for the <span
				style="text-decoration: underline;">Ultimate Client Manager</span>. <br/>This demo resets every now and then.
			<br/>The Ultimate Client Manager can be downloaded from CodeCanyon. <br/><a
				href="http://codecanyon.net/item/ultimate-client-manager-lite-edition/47626?ref=dtbaker">Please click here for
				more details</a>.
		</div>
	<?php }
								...
								if ( ! count( $todo_list ) ) {
									...
									foreach ( $todo_list as $todo_item ) {
										...
										<tr class="<?php echo ( $x ++ % 2 ) ? 'even' : 'odd'; ?>">
											<td class="row_action">
												<a
													href="<?php echo module_job::link_open( $todo_item['job_id'], false, $job_data ); ?>"><?php echo $todo_item['description']; ?></a>
											</td>
											<?php if ( $display_mode != 'mobile' ) { ?>
												<td width="5%">
