...
if ( ! extension_loaded( 'zip' ) ) {
	?>
	<p>Warning: Full backups will not work correctly because this hosting account does not have the <strong>ZIP</strong>
		PHP extension enabled. Please contact the hosting provider and ask them to enable the PHP ZipArchive class.</p>
	<?php
}
		...
		if ( ! $backup['backup_file'] ) {
          ...
          ucm.backup.file_list.push({
              ...
              status: false
					...
					if ( $fileInfo->isDot() || ! $fileInfo->isDir() || $fileInfo->getFilename() == 'temp' || $fileInfo->getFilename() == 'includes' || $fileInfo->getFilename() == '.hg' || $fileInfo->getFilename() == '.git' ) {
						continue;
					} ?>
          ucm.backup.file_list.push({
