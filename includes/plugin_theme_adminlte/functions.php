		...
		<div class="header_buttons">
			<?php foreach ( $buttons as $button ) { ?>
				<a href="<?php echo $button['url']; ?>"
				   class="btn btn-adminlte-5 btn-success btn-sm<?php echo isset( $button['class'] ) ? ' ' . $button['class'] : ''; ?>"<?php
				if ( isset( $button['id'] ) ) {
					echo ' id="' . $button['id'] . '"';
				}
	...
	if ( module_theme::get_config( 'adminlte_formstyle', 'table' ) == 'div' ) {
		...
		<input type="text" name="customer_name" value="1 New Theme Test" class="form-control  plugin_form_required">
			...
			foreach ( $matches[0] as $key => $val ) {
				...
				$html = str_replace( $val, '<div class="' . $classes . '" ' . $matches[1][ $key ] . '><div class="input-group"><span class="input-group-addon table-row-title" ' . $title_match[1] . '><span class="' . module_theme::get_config( 'adminlte_titlewidth', 'width1' ) . '">' . $title_match[2] . '</span></span>' .
...
function adminlte_generate_fieldset( $callback, $options ) {
	$defaults = array(
		...
		'title_type'      => 'h3',
		'heading'         => false,
		'row_title_class' => module_theme::get_config( 'adminlte_titlewidth', 'width1' ),
		'row_data_class'  => '',
		'elements'        => array(),
			...
			if ( module_theme::get_config( 'adminlte_formstyle', 'table' ) == 'table' ) {
					...
					foreach ( $options['elements'] as $element_id => $element ) {
						...
						?>>
							...
							<?php } else { ?>
								...
								<?php } ?>
								<td
									class="<?php echo isset( $element['row_data_class'] ) ? $element['row_data_class'] : $options['row_data_class']; ?>" <?php echo ! isset( $element['title'] ) ? ' colspan="2" ' : ''; ?>>
									<?php if ( isset( $element['fields'] ) ) { ?>
										<?php if ( is_array( $element['fields'] ) ) {
											foreach ( $element['fields'] as $dataid => $field ) {
												if ( is_array( $field ) ) {
				...
				<div class="<?php echo $options['class']; ?>">
					...
					foreach ( $options['elements'] as $element_id => $element ) {
						...
						?>
						<div class="form-group form-group-long" id="<?php echo $fieldset_id . '_' . $element_id; ?>" <?php
						if ( ! empty( $element['dependency'] ) && is_array( $element['dependency'] ) ) {
							if ( empty( $element['dependency']['fieldset_id'] ) ) {
								$element['dependency']['fieldset_id'] = $fieldset_id;
							}
						...
						?>>
							...
							<?php } else { ?>
								...
								<?php if ( isset( $element['title'] ) ) { ?>
									<label
										class="<?php echo isset( $element['row_title_class'] ) ? $element['row_title_class'] : $options['row_title_class']; ?>"><?php echo htmlspecialchars( _l( $element['title'] ) ); ?></label>
								<?php }
								if ( isset( $element['fields'] ) ) { ?>
