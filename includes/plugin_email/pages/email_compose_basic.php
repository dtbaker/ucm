		...
		'field' => function () use ( $to, $options, $headers, $to_select ) {
			...
			if ( count( $to ) > 1 ) {
				...
				<select name="custom_to">
					...
					<?php foreach ( $to as $t ) { ?>
						<option
							value="<?php echo htmlspecialchars( $t['email'] ); ?>||<?php echo htmlspecialchars( $t['name'] ) . ( isset( $t['last_name'] ) && module_config::c( 'email_to_full_name', 1 ) ? ' ' . htmlspecialchars( $t['last_name'] ) : '' ); ?>||<?php echo htmlspecialchars( $t['user_id'] ); ?>"<?php if ( isset( $to_select ) && $to_select == $t['email'] ) {
							echo ' selected';
			...
			<?php }else{ ?>
				<div id="email_to_holder">
					<div class="dynamic_block">
						<input type="email" name="custom_to[]"
						       value="<?php echo isset( $to_select ) ? htmlspecialchars( $to_select ) : ''; ?>">
						<a href="#" class="add_addit" onclick="return seladd(this);">+</a> <a href="#" class="remove_addit"
						                                                                      onclick="return selrem(this);">-</a>
					</div>
				</div>
				...
				<div id="email_cc_holder">
					<div class="dynamic_block">
						<select name="custom_cc[]">
							...
							<?php foreach ( $to as $t ) { ?>
								<option
									value="<?php echo htmlspecialchars( $t['email'] ); ?>||<?php echo htmlspecialchars( $t['name'] ) . ( isset( $t['last_name'] ) && module_config::c( 'email_to_full_name', 1 ) ? ' ' . htmlspecialchars( $t['last_name'] ) : '' ); ?>||<?php echo htmlspecialchars( $t['user_id'] ); ?>"><?php echo htmlspecialchars( $t['email'] ) . ' - ' . htmlspecialchars( $t['name'] ); ?></option>
							<?php } ?>
						</select>
						<a href="#" class="add_addit" onclick="return seladd(this);">+</a> <a href="#" class="remove_addit"
						                                                                      onclick="return selrem(this);">-</a>
