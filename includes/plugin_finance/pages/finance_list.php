	...
	<form action="" method="post" id="quick_add_form">
		...
		$columns['sort_name']           = array(
			...
			'callback' => function ( $finance ) {
				...
				?> <a
					href="<?php echo $finance['url']; ?>"><?php echo ! trim( $finance['name'] ) ? 'N/A' : htmlspecialchars( $finance['name'] ); ?></a> <?php
			},
		);
		...
		$columns['sort_credit']         = array(
			...
			'callback' => function ( $finance ) {
				...
				?> <span
					class="success_text"><?php echo $finance['credit'] > 0 ? '+' . dollar( $finance['credit'], true, $finance['currency_id'] ) : '';
					echo $info; ?></span> <?php
			},
		);
		$columns['sort_debit']          = array(
			...
			'callback' => function ( $finance ) {
				...
				?> <span
					class="error_text"><?php echo $finance['debit'] > 0 ? '-' . dollar( $finance['debit'], true, $finance['currency_id'] ) : '';
					echo $info; ?></span> <?php
			},
		);
		...
		$columns['finance_tick']        = array(
			...
			'callback' => function ( $finance ) {
				...
				if ( isset( $finance['invoice_payment_id'] ) && $finance['invoice_payment_id'] && isset( $finance['invoice_id'] ) && $finance['invoice_id'] ) { ?>
					<input type="checkbox" name="link_invoice_payment_ids[<?php echo $finance['invoice_payment_id']; ?>]"
					       value="<?php echo number_format( $finance['credit'], 2 ) . '_' . number_format( $finance['debit'], 2 ); ?>"
					       class="link_box">
				<?php } else if ( isset( $finance['finance_id'] ) ) { ?>
					<input type="checkbox" name="link_finance_ids[<?php echo $finance['finance_id']; ?>]"
					       value="<?php echo number_format( $finance['credit'], 2 ) . '_' . number_format( $finance['debit'], 2 ); ?>"
					       class="link_box">
				<?php }
			},
		...
		if ( module_finance::can_i( 'create', 'Finance' ) ) {
			...
			ob_start();
			?>
			<div style="height:18px; width:89px; overflow: hidden; position: absolute; background: #FFFFFF;"
			     onmouseover="$(this).height('auto');$(this).width('auto');"
			     onmouseout="$(this).height('18px');$(this).width('89px');">
				<?php
