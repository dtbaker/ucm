...
<form action="" method="post">
	...
	<table class="tableclass tableclass_rows">
		...
		<?php foreach ( get_multiple( 'config', false, 'key', 'exact', '`key`' ) as $config ) {
			...
			if ( _DEMO_MODE && strpos( $config['key'], 'smtp' ) !== false ) {
				continue;
			}
			?>
			<tr>
				...
				<td>
					<span data-name="config[<?php echo htmlspecialchars( $config['key'] ); ?>]"
					      class="config_variable"><?php echo htmlspecialchars( $config['val'] ); ?></span>
				</td>
			</tr>
		<?php } ?>
		<tr>
