...
if ( $data_type_id ) {
	...
	if ( isset( $_REQUEST['search_go'] ) && $_REQUEST['search_go'] ) {
		...
		<form action="" method="POST">
          ...
          $(function () {
			...
			<input type="button" name="cancel" value="Cancel" class="submit_button"
			       onclick="window.location.href='<?php echo $module->link( '', array(
				       'data_type_id' => $data_type_id,
				       'view_all'     => isset( $_REQUEST['view_all'] ) ? 1 : false,
			       ) ); ?>';">
		</form>
	...
	<?php foreach ( $data_types as $data_type ) {
		...
		<a class="uibutton"
		   href="<?php echo $module->link( 'admin_data_search', array( 'data_type_id' => $data_type['data_type_id'] ) ); ?>"><?php echo $data_type['data_type_name']; ?></a>
