			...
			if ($maincontent.data('wl_Widget').currentid++ >= $maincontent.data('wl_Widget').containercount) {
				...
				$container.each(function (i, cont) {
					...
					$.each(widgets, function (widget, options) {
						...
						(options.collapsed && typeof _widget.data('wl_Widget') != 'undefined' && typeof _widget.data('wl_Widget').collapsible != 'undefined' && _widget.data('wl_Widget').collapsible) ? _widget.addClass('collapsed').find('div').eq(0).hide().data('wl_Widget', {
							collapsed: true
