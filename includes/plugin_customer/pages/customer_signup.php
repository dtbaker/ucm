...
<table width="100%">
	...
	<tr>
		...
		<td valign="top">
			<p>
				On the left is an example signup form - your customers can complete this form to input their details directly
				into your system - handy! <br> You can copy &amp; paste the HTML code onto your website.
			...
			<p>
				Or if you are happy with the default signup form then you can use it at this address here: <a
					href="<?php echo module_customer::link_public_signup_form(); ?>"
					target="_blank"><?php echo module_customer::link_public_signup_form(); ?></a>.
			</p>
			<p>
				The best way to see how this works is to fill in the example form then have a look through your system to see
				how that information is inserted. Below is the information that is inserted into the system from this form:

			</p>
			<ul>
				<li>A Customer with the customer name from the form (new or existing customer, depending on the 'matching'
					setting above)
				</li>
				...
				<li>If there are any "custom" fields for the Customer or Website you will see them here in the form. These also
					get added to the system respectively. After adding new custom fields please come back here to regenerate the
					HTML code for your website.
				</li>
				<li>An email will be sent to the customer (see Settings > Template to configure this email)</li>
				<li>An email will be sent to the administrator with details of the signup (see Settings > Template to configure
					this email)
				</li>
				<li>A thank you message will be displayed after submitting the form (see Settings > Template to configure this
					message)
				</li>
			</ul>
			<p>
				Below is the HTML code for the form on the left, including some sample styles which you can adjust to match your
				own website.
			</p>
			<p>
				If you want to track different customer types please add the corresponding hidden input field to the page as
				well:<br>
				<?php foreach ( module_customer::get_customer_types() as $type ) { ?>
					&lt;input type="hidden" name="customer[customer_type_id]" value="<?php echo $type['customer_type_id']; ?>"&gt;  (for <?php echo htmlspecialchars( $type['type_name_plural'] ); ?>)
					<br/>
				<?php } ?>
			</p>
			<p>
				If you want to submit this form via ajax using jQuery, set a field called 'via_ajax' and any errors will be
				returned via json array.
			</p>
			<textarea rows="3" cols="3"
			          style="width:90%; height: 500px"><?php echo htmlspecialchars( $form_html ); ?></textarea>
		</td>
	</tr>
	</tbody>
</table>