			...
			<ol>
				...
				foreach ( module_extra::get_defaults( 'customer' ) as $default ) { ?>
					<li>
						...
						<input type="text" id="customer_extra_<?php echo $x; ?>"
						       name="customer[extra][<?php echo htmlspecialchars( $default['key'] ); ?>]"/>
					</li>
					<?php $x ++;
				}
				$x = 1;
				foreach ( module_group::get_groups( 'customer' ) as $group_data ) { ?>
					<li>
						...
						<input id="customer_group_<?php echo $x; ?>"
						       name="customer[group_ids][<?php echo htmlspecialchars( $group_data['group_id'] ); ?>]"
						       type="checkbox" value="<?php echo htmlspecialchars( $group_data['group_id'] ); ?>"/> Yes
					</li>
					<?php $x ++;
				} ?>
			</ol>
			...
			<fieldset>
				...
				<ol>
					<li>
						...
						<input type="hidden" name="contact[name][<?= $x; ?>_required]"
						       value="<?php echo $x == 0 ? 'Primary ' : 'Additional '; ?>Contact First Name"/>
					</li>
					<li>
					...
					<li>
						...
						<input type="hidden" name="contact[email][<?= $x; ?>_required]"
						       value="<?php echo $x == 0 ? 'Primary ' : 'Additional '; ?>Contact Email Address"/>
					</li>
					<li>
					...
					foreach ( module_extra::get_defaults( 'user' ) as $default ) { ?>
						<li>
							<label
								for="contact_extra_<?= $x; ?>_<?php echo $xx; ?>"><?php echo htmlspecialchars( $default['key'] ); ?></label>
							<input type="text" id="contact_extra_<?= $x; ?>_<?php echo $xx; ?>"
							       name="contact[extra][<?= $x; ?>][<?php echo htmlspecialchars( $default['key'] ); ?>]"/>
						</li>
						<?php $xx ++;
					}
					$xx = 1;
					foreach ( module_group::get_groups( 'user' ) as $group_data ) { ?>
						<li>
							<label
								for="contact_group_<?= $x; ?>_<?php echo $xx; ?>"><?php echo htmlspecialchars( $group_data['name'] ); ?></label>
							<input id="contact_group_<?= $x; ?>_<?php echo $xx; ?>"
							       name="contact[group_ids][<?= $x; ?>][<?php echo htmlspecialchars( $group_data['group_id'] ); ?>]"
							       type="checkbox" value="<?php echo htmlspecialchars( $group_data['group_id'] ); ?>"/> Yes
						</li>
						<?php $xx ++;
					} ?>
			...
			<ol>
				...
				<li>
					<fieldset>
						...
						foreach ( module_job::get_types() as $type_id => $type ) {
							$has_type = true; ?>
							<label><input type="checkbox" name="job[type][<?php echo htmlspecialchars( $type_id ); ?>]"
				...
				foreach ( module_extra::get_defaults( 'job' ) as $default ) { ?>
					<li>
						...
						<input type="text" id="job_extra_<?php echo $x; ?>"
						       name="job[extra][<?php echo htmlspecialchars( $default['key'] ); ?>]"/>
					</li>
					<?php $x ++;
				}
				?>
				...
				foreach ( module_extra::get_defaults( 'website' ) as $default ) { ?>
					<li>
						...
						<input type="text" id="website_extra_<?php echo $x; ?>"
						       name="website[extra][<?php echo htmlspecialchars( $default['key'] ); ?>]"/>
					</li>
					<?php $x ++;
				}
				$x = 1;
				foreach ( module_group::get_groups( 'website' ) as $group_data ) { ?>
					<li>
						...
						<input type="text" id="website_group_<?php echo $x; ?>"
						       name="website[group_ids][<?php echo htmlspecialchars( $group_data['group_id'] ); ?>]" type="checkbox"
						       value="<?php echo htmlspecialchars( $group_data['group_id'] ); ?>"/> Yes
					</li>
					<?php $x ++;
				}
						...
						<fieldset>
							...
							foreach ( $sorted_subscriptions as $subscription ) {
								...
								<label for="subscription_<?php echo $subscription['subscription_id']; ?>">
									<input type="checkbox"
									       name="subscription[subscriptions][<?php echo $subscription['subscription_id']; ?>]" value="1"
									       id="subscription_<?php echo $subscription['subscription_id']; ?>">
									<?php echo htmlspecialchars( $subscription['name'] ); ?>
									- <?php echo dollar( $subscription['amount'] );
