...
if(!$customer_id || !$customer_data)die('No customer Id');
	...
	<table>
		...
		foreach($invoices as $invoice){
			...
			if ($invoice['deposit_job_id'] > 0) {
				$transactions[] = array('id' => $invoice['invoice_id'], 'name' => $invoice['name'].' Deposit invoice<br/>(payments assigned directly to invoice)', 'date' => $dd, 'amount' => $invoice['c_total_amount'], 'dd' => $ddd, 'deposit' => $invoice['deposit_job_id'], 'charge' => $invoice['c_total_amount'], 'class' => "deposit", 'credit' => 0);
			} else {
				$transactions[] = array('id' => $invoice['invoice_id'], 'name' => $invoice['name'], 'date' => $dd, 'amount' => $invoice['c_total_amount'], 'dd' => $ddd, 'deposit' => $invoice['deposit_job_id'], 'charge' => $invoice['c_total_amount'], 'class' => "invoice", 'credit' => 0);
			}
		}
		...
		foreach($transactions2 as $transaction) {
			...
			<tr>
				...
				<td class="">
					<a href="<?php echo module_invoice::link_public( $transaction['id'] );?>" target="_blank"><?php echo htmlspecialchars($transaction['name']);?></a>
				</td>
