...
class ucm_twitter {
	...
	public function output_row( $message, $settings ) {
		...
		?>
		<tr
			class="<?php echo isset( $settings['row_class'] ) ? $settings['row_class'] : ''; ?> twitter_message_row <?php echo ! isset( $message['read_time'] ) || ! $message['read_time'] ? ' message_row_unread' : ''; ?>"
				...
				<div class="social_from_holder social_twitter">
					<div class="social_from_full">
						...
						foreach ( $from as $id => $from_data ) {
							?>
							<div>
								<a href="//twitter.com/<?php echo htmlspecialchars( $from_data['screen_name'] ); ?>"
			...
			<td class="social_column_summary">
				<div
					class="twitter_message_summary<?php echo ! isset( $message['read_time'] ) || ! $message['read_time'] ? ' unread' : ''; ?>"> <?php
...
class ucm_twitter_account {
	...
	public function update( $field, $value ) {
		...
		if ( in_array( $field, array( 'social_twitter_id' ) ) ) {
			return;
		}
		if ( $this->social_twitter_id ) {
			$this->{$field} = $value;
	...
	public function import_data( $debug = false ) {
		...
		foreach ( $latest_search_values as $key => $val ) {
			if ( isset( $searches_data[ $key ] ) ) {
				$latest_search_values[ $key ] = $searches_data[ $key ];
			}
		}
		...
		if ( $code == 200 ) {
			...
			if ( isset( $data['status'] ) ) {
				...
				if ( $debug ) {
					echo " Hello @" . htmlspecialchars( $data['screen_name'] ) . ", importing information...<br>\n";
				}

				if ( $this->get( 'import_dm' ) ) {
					$timestart = microtime( true );
					if ( $debug ) {
						echo " Importing sent direct messages (after tweet " . $latest_search_values['dm_sent'] . ")<br>\n";
					}
					...
					if ( $code_sent == 200 ) {
						...
						foreach ( $dms_sent as $dm ) {
							if ( isset( $dm['id_str'] ) ) {
								if ( $debug ) {
									echo ' - importing DM ID: ' . $dm['id_str'] . "<br>\n";
								}
								$latest_search_values['dm_sent'] = max( $latest_search_values['dm_sent'], $dm['id_str'] );
								// does this exist in the database?
								$exists = get_single( 'social_twitter_message', array(
									'social_twitter_id',
									'twitter_message_id'
								), array( $this->social_twitter_id, $dm['id_str'] ) );
								...
								update_insert( 'social_twitter_message_id', $exists && isset( $exists['social_twitter_message_id'] ) ? $exists['social_twitter_message_id'] : false, 'social_twitter_message', array(
									...
									'summary'            => isset( $dm['text'] ) ? $dm['text'] : '',
									//todo: swap out shortened urls in 'entities' array.
					...
					if ( $debug ) {
						echo " - took: " . ( microtime( true ) - $timestart ) . " <br>\n";
					}

					$timestart = microtime( true );
					if ( $debug ) {
						echo " Importing received direct messages (after tweet " . $latest_search_values['dm_received'] . ")<br>\n";
					}
				...
				if ( $this->get( 'import_mentions' ) ) {
					$timestart = microtime( true );
					if ( $debug ) {
						echo " Importing mentions (after tweet " . $latest_search_values['mentions'] . ")<br>\n";
					}
					$code_sent = $tmhOAuth->user_request( array(
					...
					if ( $code_sent == 200 ) {
						...
						foreach ( $mentions as $mention ) {
							if ( isset( $mention['id_str'] ) ) {
								...
								if ( $debug ) {
									echo ' - importing mention ID: ' . $mention['id_str'] . "<br>\n";
								}
								/*// does this exist in the database?
				...
				if ( $this->get( 'import_tweets' ) ) {
					$timestart = microtime( true );
					if ( $debug ) {
						echo " Importing my own tweets (after tweet " . $latest_search_values['timeline'] . ")<br>\n";
					}
					$code_sent = $tmhOAuth->user_request( array(
					...
					if ( $code_sent == 200 ) {
						...
						foreach ( $tweets as $tweet ) {
							if ( isset( $tweet['id_str'] ) ) {
								...
								if ( $debug ) {
									echo ' - importing tweet ID: ' . $tweet['id_str'] . "<br>\n";
								}
					...
					$timestart = microtime( true );
					if ( $debug ) {
						echo " Importing retweets of my tweets (after tweet " . $latest_search_values['retweets'] . ")<br>\n";
					}
					$code_sent = $tmhOAuth->user_request( array(
				...
				if ( $debug ) {
					echo 'Twitter failed to check status, please try connecting to twitter again from settings: ' . $tmhOAuth->response['response'];
				}
			...
			if ( $debug ) {
				echo 'Twitter failed to check authentication, please try connecting to twitter again from settings: ' . $tmhOAuth->response['response'];
			}
...
class ucm_twitter_message {
	...
	public function load_by_twitter_id( $twitter_id, $tweet = false, $type = false, $debug = false, $force = false ) {
		...
		$exists                          = get_single( 'social_twitter_message', array(
			'social_twitter_id',
			'twitter_message_id'
		), array( $this->twitter_account->get( 'social_twitter_id' ), $twitter_id ) );
		...
		if ( $tweet ) {
			...
			$this->social_twitter_message_id = update_insert( 'social_twitter_message_id', $this->social_twitter_message_id, 'social_twitter_message', array(
				...
				'summary'            => isset( $tweet['text'] ) ? $tweet['text'] : '',
				//todo: swap out shortened urls in 'entities' array.
				'message_time'       => isset( $tweet['created_at'] ) ? strtotime( $tweet['created_at'] ) : '',
				'data'               => json_encode( $tweet ),
				'type'               => $type,
	...
	public function parse_links() {
		if ( ! $this->get( 'social_twitter_message_id' ) ) {
			return;
		}
	...
	public function output_block( $level ) {
		...
		if ( $this->get( 'summary' ) ) {
			...
			?>
			<div class="twitter_comment <?php echo $level != 0 ? ' twitter_comment_clickable' : 'twitter_comment_current'; ?>"
			     data-id="<?php echo $this->social_twitter_message_id; ?>"
			     data-link="<?php echo module_social_twitter::link_open_twitter_message( $this->get( 'social_twitter_id' ), $this->social_twitter_message_id ); ?>"
			     data-title="<?php echo _l( 'Tweet' ); ?>">
			...
			<div class="twitter_comment twitter_comment_reply_box twitter_comment_reply_box_level<?php echo $level; ?>">
				...
				<div class="twitter_comment_reply">
					<textarea placeholder="Write a reply..." class="twitter_compose_message"><?php
						...
						?></textarea>
					<button data-id="<?php echo (int) $this->social_twitter_message_id; ?>"
					        data-account-id="<?php echo (int) $this->get( 'social_twitter_id' ); ?>"><?php _e( 'Send' ); ?></button>
