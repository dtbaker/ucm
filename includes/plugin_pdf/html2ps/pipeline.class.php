...
class Pipeline {
	...
	function _reset_page_at_rules() {
		$this->_page_at_rules = array(
			CSS_PAGE_SELECTOR_ALL   => array(),
	...
	function _show_item( &$box, $offset, &$context, &$media, &$postponed_filter ) {
		...
		$this->_dispatcher->fire( 'before-page-heights', array(
			'pipeline' => &$this,
			'document' => &$box,
			'media'    => &$media
		) );
		...
		$this->_dispatcher->fire( 'before-document', array(
			'pipeline'     => &$this,
			'document'     => &$box,
		...
		for ( $i = 0; $i < $expected_pages; $i ++ ) {
			...
			$this->_dispatcher->fire( 'before-page', array(
				'pipeline' => &$this,
				'document' => &$box,
				'pageno'   => $i
			) );
			...
			$this->_dispatcher->fire( 'after-page', array(
				'pipeline' => &$this,
				'document' => &$box,
				'pageno'   => $i
			) );
		...
		$this->_dispatcher->fire( 'after-document', array(
			'pipeline' => &$this,
			'document' => &$box
		) );
	}
	...
	function &_layout_item( $data_id, &$media, $offset, &$context, &$postponed_filter ) {
		...
		$this->_dispatcher->fire( 'after-parse', array(
			'pipeline' => &$this,
			'document' => &$box,
			'media'    => $media
		) );
