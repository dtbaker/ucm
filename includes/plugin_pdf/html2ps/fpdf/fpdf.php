...
if ( ! class_exists( 'FPDF' ) ) {
	...
	class PDFAnnotationExternalLink extends PDFAnnotation {
		...
		function PDFAnnotationExternalLink(
			&$handler,
			$object_id,
			$generation_id,
			$rect,
	...
	class PDFAnnotationInternalLink extends PDFAnnotation {
		...
		function PDFAnnotationInternalLink(
			&$handler,
			$object_id,
			$generation_id,
			$rect,
	...
	class PDFFieldGroup extends PDFIndirectObject {
		...
		function _dict( &$handler ) {
			return array_merge( parent::_dict( $handler ),
				array(
					"Kids" => $handler->_reference_array( $this->_kids ),
	...
	class PDFFieldPushButtonImage extends PDFFieldPushButton {
		...
		function PDFFieldPushButtonImage(
			&$handler,
			$object_id,
			$generation_id,
			$rect,
	...
	class PDFFieldPushButtonSubmit extends PDFFieldPushButton {
		...
		function PDFFieldPushButtonSubmit(
			&$handler,
			$object_id,
			$generation_id,
			$rect,
	...
	class PDFFieldText extends PDFField {
		...
		function _dict( &$handler ) {
			return array_merge( parent::_dict( $handler ),
				array(
					'FT' => '/Tx',
					'V'  => $handler->_textstring( $this->_value ),
					// Current value
					'DV' => $handler->_textstring( $this->_value ),
					// Default value
					'DR' => "2 0 R",
	...
	class FPDF {
		...
		function _putfonts() {
			...
			foreach ( $this->diffs as $diff ) {
				//Encodings
				$this->_newobj();
				$this->_out( $this->_dictionary( array(
					"Type"         => "/Encoding",
