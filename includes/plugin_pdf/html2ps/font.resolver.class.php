...
class FontResolver {
	...
	function add_family_normal_encoding_override_mask( $family, $encoding, $normal, $italic, $oblique ) {
		$this->overrides_mask[ $family ][ WEIGHT_NORMAL ][ FS_NORMAL ][]  = array(
			'mask'     => $encoding,
			'override' => $normal
		);
		$this->overrides_mask[ $family ][ WEIGHT_NORMAL ][ FS_ITALIC ][]  = array(
			'mask'     => $encoding,
			'override' => $italic
		);
		$this->overrides_mask[ $family ][ WEIGHT_NORMAL ][ FS_OBLIQUE ][] = array(
			'mask'     => $encoding,
			'override' => $oblique
		);
	}
	...
	function add_family_bold_encoding_override_mask( $family, $encoding, $normal, $italic, $oblique ) {
		$this->overrides_mask[ $family ][ WEIGHT_BOLD ][ FS_NORMAL ][]  = array(
			'mask'     => $encoding,
			'override' => $normal
		);
		$this->overrides_mask[ $family ][ WEIGHT_BOLD ][ FS_ITALIC ][]  = array(
			'mask'     => $encoding,
			'override' => $italic
		);
		$this->overrides_mask[ $family ][ WEIGHT_BOLD ][ FS_OBLIQUE ][] = array(
			'mask'     => $encoding,
			'override' => $oblique
		);
	}
