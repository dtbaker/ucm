...
', 'Change Request: {WEBSITE_NAME}', array(
	...
	public static function link_open_delete( $change_request_id, $full = false, $data = array() ) {
		return self::link_generate( $change_request_id, array(
			'full' => $full,
			'data' => $data,
			'page' => 'change_request_delete'
		) );
	}
	...
	public static function get_change_request_by_website( $website_id, $change_request_id ) {
		$cr                = get_single( 'change_request', array( 'website_id', 'change_request_id' ), array(
			$website_id,
			$change_request_id
		) );
	...
	public static function hook_website_main( $callback_name, $website_id ) {
		if ( (int) $website_id > 0 && self::can_i( 'view', 'Change Requests' ) ) {
			// check if this website is linked to any change_request payments.
			$change_requests        = self::get_change_requests( array(
					'website_id' => $website_id,
					'status'     => '1'
				) ) + self::get_change_requests( array( 'website_id' => $website_id, 'status' => '2' ) );
			$change_request_website = get_single( 'change_request_website', 'website_id', $website_id );
	...
	public static function get_remaining_changes( $website_id ) {
		...
		$changes                = array(
			0,
			isset( $change_request_website['limit_number'] ) ? $change_request_website['limit_number'] : 5
		);
    ...
    ', 'Displayed after a change request is created/updated.', 'code' );
						...
						case 'display_change':
							...
							<div class="content">
								...
								<div class="wp3changerequest_actions">
									<p>
                                        ...
                                        <?php } ?>
                                        <br/>-->
										<strong><?php _e( 'Created by:' ); ?></strong> <?php echo htmlspecialchars( $change_request['name'] ); ?>
										<br/>
									...
									<?php if ( ! isset( $change_request['job_id'] ) || ! $change_request['job_id'] || self::can_i( 'edit', 'Change Requests' ) ) { ?>
										<p align="center">
											<input type="button" name="edit" value="<?php _e( 'Edit' ); ?>"
											       class="wp3changerequest_button wp3changerequest_button_small"
											       onclick="dtbaker_changerequest.edit(<?php echo $change_request_id; ?>); return false;">
										</p>
									<?php } ?>
								</div>
							</div>
							...
							jQuery('body').prepend('
							<div class="wp3changerequest_change" id="dtbaker_change_'+change_id+'"
							     style="'+((!t.show_postits) ? 'display:none;':'')+'"></div>');
						...
						default:
							...
							// fix for jQuery 1.9+
							jQuery('body').append('
							<div id="dtbaker_changerequest_inlinewizard" style="display:none;"></div>');
							}
			...
			case 'script':
				...
				if ( $website_id && $hash && module_change_request::link_script( $website_id, true ) == $hash ) {
					...
					if ( $client_url ) {
						...
						$completed_change_requests = self::get_change_requests( array(
							'website_id' => $website_id,
							'status'     => _CHANGE_REQUEST_STATUS_COMPLETE
						) );
						?>
