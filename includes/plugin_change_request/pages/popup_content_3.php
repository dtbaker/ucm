...
<form action="<?php echo module_change_request::link_popup( $website_id ); ?>&type=save" method="post"
      enctype="multipart/form-data" id="change_request_submit_form">


	<input type="hidden" name="change_id"
	       value="<?php echo isset( $change_request['change_request_id'] ) ? $change_request['change_request_id'] : 0; ?>">
	...
	<input type="hidden" name="window_width"
	       value="<?php echo isset( $change_request['window_width'] ) ? $change_request['window_width'] : 0; ?>">
	<input type="hidden" name="url"
	       value="<?php echo isset( $change_request['url'] ) ? htmlspecialchars( $change_request['url'] ) : ''; ?>">
		...
		<tr>
			...
			<td>
				<?php if ( isset( $change_request['job_id'] ) && $change_request['job_id'] ) {
					...
					<textarea rows="5" cols="30" name="request" style="width:100%;"
					          class="wp3changerequest_input"><?php echo htmlspecialchars( $change_request['request'] ); ?></textarea>
				<?php } ?>
			</td>
		</tr>
		<!-- <tr>
		...
		<tr>
			...
			<td>
				<?php if ( isset( $change_request['job_id'] ) && $change_request['job_id'] ) {
					...
					<input type="text" name="name" class="wp3changerequest_input"
					       value="<?php echo htmlspecialchars( $change_request['name'] ); ?>">
				<?php } ?>
			</td>
		</tr>
	</table>
			...
			<table>
				<tr>
					...
					<td>
						...
						<label
							for="change_completed_email"><?php _e( 'Email client letting them know this change has been completed.' ); ?></label>
		</div>
		</td>
		</tr>
		</table>
	...
	<p>
		<?php if ( $change_id ) { ?>
			<?php if ( ! isset( $change_request['job_id'] ) || ! $change_request['job_id'] ) { ?>
				<input type="submit" name="delete_request" value="<?php _e( 'Delete' ); ?>"
				       class="wp3changerequest_button wp3changerequest_button_cancel"
				       onclick="return confirm('<?php _e( 'Really delete this?' ); ?>');">
			<?php } ?> <input type="submit" name="submit_request" value="<?php _e( 'Save Changes &raquo;' ); ?>"
			                  class="wp3changerequest_button tour_5">
		<?php } else { ?>
			<input type="submit" name="submit_request" value="<?php _e( 'Submit Request &raquo;' ); ?>"
			       class="wp3changerequest_button tour_5">
		<?php } ?>
