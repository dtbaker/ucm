...
<?php /* <script type="text/javascript">
	...
	<table border="0" cellspacing="0" cellpadding="2" class="tableclass tableclass_rows tableclass_full">
		...
		$c = 0;
		foreach ( $change_requests as $change_request ) {
			if ( ! $show_completed && $change_request['status'] == _CHANGE_REQUEST_STATUS_COMPLETE ) {
				continue;
			}
			?>
			<tr class="<?php echo $c ++ % 2 ? 'odd' : 'even'; ?>">
				...
				<td>
					<?php if ( strlen( $change_request['request'] ) > 40 ) { ?>
						<a href="#"
						   onclick="$(this).next().show();$(this).hide();return false;"><?php echo substr( $change_request['request'], 0, 40 ); ?>
							....</a>
				...
				<td>
					<?php switch ( $change_request['status'] ) {
						case _CHANGE_REQUEST_STATUS_NEW:
							_e( 'Incomplete' );
							break;
						case _CHANGE_REQUEST_STATUS_COMPLETE:
							_e( 'Completed' );
							break;
					} ?>
				</td>
				<td>
					<?php if ( $change_request['job_id'] ) {
						...
						if ( ! $change_request['task_id'] || ! isset( $tasks[ $change_request['task_id'] ] ) ) {
							...
							update_insert( 'change_request_id', $change_request['change_request_id'], 'change_request', array( 'job_id'  => 0,
							                                                                                                   'task_id' => 0
							) );
						}
					}
					if ( $change_request['job_id'] ) {
						...
						if ( count( $jobs ) ) {
							...
							?>
							@
							<input type="text" name="add_job_hours"
							       value="<?php echo module_config::c( 'change_request_job_hours', 1 ); ?>" class="add_job_hours"
							       style="width:15px;"><?php _e( 'hrs' ); ?>
							<input type="button" name="add_job" class="form_save" value="<?php _e( 'Add' ); ?>"
							       onclick="$('#add_change_request_to_website_job_id').val($(this).parent().find('select').first().val());$('#add_change_request_to_website_hours').val($(this).prev('.add_job_hours').val()); $('#add_change_request_to_website').val(<?php echo $change_request['change_request_id']; ?>); this.form.submit(); ">
							<?php
						} else {
				...
				<td>
					<a
						href="<?php echo module_change_request::link_public_change( $website_id, $change_request['change_request_id'] ); ?>"
						target="_blank"><?php _e( 'View' ); ?></a>
					|
					<a
						href="<?php echo module_change_request::link_open( $change_request['change_request_id'] ); ?>"><?php _e( 'Email' ); ?></a>
					<?php if ( module_change_request::can_i( 'delete', 'Change Requests' ) && ! $change_request['job_id'] ) { ?>
						|
						<a
							href="<?php echo module_change_request::link_open_delete( $change_request['change_request_id'] ); ?>"><?php _e( 'Delete' ); ?></a>
					<?php } ?>
